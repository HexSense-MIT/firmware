...
static uint16_t calcCRC(const uint8_t dataBytes[], uint8_t byteCount) {
  return calcCRCWithInitVal(dataBytes, byteCount, 0xFFFF);
}
...
static uint16_t calcCRCWithInitVal(const uint8_t dataBytes[], uint8_t byteCount, uint16_t initVal) {
  ...
  #ifdef ADS131M04_CRC_CCITT
  const uint16_t polynomial = 0x1021;
  #elif defined(ADS131M04_CRC_ANSI)
  const uint16_t polynomial = 0x8005;
  #endif

  for (uint16_t i = 0; i < byteCount; i++) {
    for (uint8_t j = 0; j < 8; j++) {
      dataMsb = (bool)((dataBytes[i] >> (7 - j)) & 0x01);
...
void ADS131M04_WriteRegister(uint8_t addr, uint16_t value) {
  ...
  for(uint8_t i = 0; i < 9; i++) {
    ADS131M04_TransmitWord16(ADS131M04_OPCODE_NULL);
  }
